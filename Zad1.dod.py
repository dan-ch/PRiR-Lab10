...
def calka(x):
  with tf.device('/device:GPU:0'):
    dx = (x[-1] - x[0]) / len(x)
    return (14*(funkcja(x[0])+funkcja(x[-1])) + 64*tf.reduce_sum(funkcja(x[1:-1:2])) + 24*tf.reduce_sum(funkcja(x[2:-1:4])) + 28*tf.reduce_sum(funkcja(x[4:-1:4])))*dx/45
...
x = tf.linspace(a, b, 45*int(input("Podaj ilosc podziałów: ")))
